...
void Top_Export(DsExport_top &ds_top,DsVattu ds ,int begin,int end){
	cout<<"dit me sao deo vao dc ??";
	memset(dd,0,sizeof(dd));
	init_array(dda,DataVattu.num);
	
	//DsExport_top ds_top;
	cout<<"datanhanvien"<<DataNhanvien.num<<endl;
...
void Xuat_export_top(Export_top ex,int x,int y,int c_line[]){
	...
	outtextxy(x+c_line[2],y,ex.soluong);
}
